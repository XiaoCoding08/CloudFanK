<template>
  ...
  <el-form :model="form" >
    <el-form-item>
      <el-input
            placeholder="输入班级号" v-model="form.classid"
          />
    </el-form-item>
  ...
  <el-table :data="tableData" style="width: 200dvh; height: 80dvh;">
    ...
    <el-table-column label="课程号" width="250">
      <template #default="scope">
        <el-popover effect="light" trigger="hover" placement="top" width="auto">
          <template #default>
            <div>courseId: {{ scope.row.courseId }}</div>
          </template>
          <template #reference>
...
onMounted(() => { 
  httpInstance.post('/student/getClass', requestData2)
  .then(response => {
...
if (
  form.value.classid.trim() === '' 
  
) {
  // 如果有任何一个字段为空，可以给出提示或者阻止注册逻辑继续执行
  //console.log('请填写完整的注册信息');
  if( form.value.classid.trim() === '')
  {
    console.log('请填写classid的注册信息');
  }
  return;
}
...
const requestData = {
  ...
  httpInstance.post('/student/joinClass', requestData)
  .then(response => {
    console.log(response.data);
    // 注册成功，可以根据实际需求进行处理，比如跳转到登录页面
    alert("创建成功");
...
const handleEdit1 = (index: number, row: User) => {
  //alert("qqqqqqqqq");
...
httpParams.post('/teacher/delOneCourse',
{
