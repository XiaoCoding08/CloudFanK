<template>
    ...
    <el-form :model="form" >
      <el-form-item>
        <el-input
              placeholder="输入课程名" v-model="form.coursename"
            />
      </el-form-item>
  ...
  import {useidStore,course_classinfoStore} from '@/stores/counter'
  ...
  onMounted(() => { 
    httpInstance.post('/teacher/queryBatchCourse', requestData2)
    .then(response => {
  ...
  if (
    ...
    {
      console.log('请填写coursename的注册信息');
    }
    return;
  }
  ...
  const requestData = {
    ...
    httpInstance.post('/teacher/addCourse', requestData)
    .then(response => {
      console.log(response.data);
      // 注册成功，可以根据实际需求进行处理，比如跳转到登录页面
      alert("创建成功");
